# Ported by ilham from Friday Userbot & Panda Userbott
...
from PunyaAlby import CMD_HELP, CMD_LIST, XTRA_CMD_LIST, Config, albyversion, pyrobot
from PunyaAlby import pyrotgbot as bot
from PunyaAlby._func._helpers import cb_wrapper, get_all_pros, humanbytes, paginate_help
from PunyaAlby._func.assistant_helpers import download_yt
from PunyaAlby._func.startup import run_cmd

milikalby = "https://telegra.ph/file/336d811bce4f1d1053fbf.jpg"
Petercord_Userbot = pyrobot
db_m = TinyDB("./PunyaAlby/resources/secret.json")
db_s = TinyDB("./PunyaAlby/resources/not4u.json")

Petercord_version = albyversion
...
**Views :** `{views}`
        ...
        results = []
        r = requests.get(
            "https://api.github.com/search/repositories", params={"q": input_}
        )
        lool = r.json()
...
"""
        ...
        await client.answer_inline_query(inline_query.id, cache_time=0, results=ok_s)
    elif string_given.startswith("pmalbyuserbot"):
        msg = (
            "❏ 𝐏𝐞𝐫𝐢𝐧𝐠𝐚𝐭𝐚𝐧!\n"
        ...
        bttn = [
            [InlineKeyboardButton(text="☑️Terima☑️", callback_data=f"terimapm")],
            [InlineKeyboardButton(text="❌Tolak❌", callback_data=f"tolakpm")],
        ]
        ...
        bttn = [
            ...
            [InlineKeyboardButton(text="🔃 System 🔃", callback_data=f"make_alby_bot")],
            [InlineKeyboardButton(text="🚫 Close 🚫", callback_data=f"cleuse")],
        ]
        if Config.LOAD_UNOFFICIAL_PLUGINS:
            total_ = len(CMD_HELP)
        nice_text = f"**📍 ALBY-PYROBOT Inline** \n\n**Version :** `{albyversion}` \n**Pyrogram Version :** `{__version__}` \n**Modules :** {total_}"
        await client.answer_inline_query(
            inline_query.id,
            cache_time=0,
            results=[
...
@bot.on_callback_query(filters.regex(pattern="make_alby_bot"))
@cb_wrapper
async def cmd_buutton(client, cb):
    bttn = [
        [InlineKeyboardButton(text="♻ Update Now ♻", callback_data=f"updTe_bot")],
        [InlineKeyboardButton(text="📳 Restart Now 📳", callback_data=f"restart_bot")],
        [InlineKeyboardButton(text="Back 🔙", callback_data=f"backO_to_help_menu")],
    ]
    await cb.edit_message_text(
        "🎗 Data ALBY-PYROBOT 🎗", reply_markup=InlineKeyboardMarkup(bttn)
    )
...
async def black_menu(client, cb):
    ...
    bttn = [
        ...
        [
            InlineKeyboardButton(
                text="🎗 Data ALBY-PYROBOT 🎗", callback_data=f"make_alby_bot"
            )
        ],
        [InlineKeyboardButton(text="🚫 Close 🚫", callback_data=f"cleuse")],
    ]
...
async def roaststart(client, cb):
    bttn = [[InlineKeyboardButton(text="Back 🔙", callback_data=f"backO_to_help_menu")]]
    await cb.edit_message_text(
        "`Please Wait, Restarting... This May Take A While`",
        reply_markup=InlineKeyboardMarkup(bttn),
    )
...
async def update_it(client, cb):
    bttn = [[InlineKeyboardButton(text="Back 🔙", callback_data=f"backO_to_help_menu")]]
    await cb.edit_message_text(
        "`Updating Please Wait!`", reply_markup=InlineKeyboardMarkup(bttn)
    )
    try:
        repo = Repo()
    except GitCommandError:
        return await cb.edit_message_text(
            "`Invalid Git Command. Please Report This Bug To @ruangdiskusikami`",
            reply_markup=InlineKeyboardMarkup(bttn),
        )
    ...
    if not Config.HEROKU_URL:
        ...
        await cb.edit_message_text(
            "`Updated Sucessfully! Give Me A min To Restart!`",
            reply_markup=InlineKeyboardMarkup(bttn),
        )
        args = [sys.executable, "-m", "PunyaAlby"]
        execle(sys.executable, *args, environ)
        ...
        await cb.edit_message_text(
            "`Heroku Detected! Pushing, Please Halt!`",
            reply_markup=InlineKeyboardMarkup(bttn),
        )
...
async def close_it_please(client, cb):
    bttn = [
        ...
        [InlineKeyboardButton(text="⛔ Hapus Semua ⛔", callback_data=f"mansiz")],
    ]
    await cb.edit_message_text(
        "**Closed Help Menu**!", reply_markup=InlineKeyboardMarkup(bttn)
    )
...
async def give_plugin_cmds(client, cb):
    ...
    help_string += "\n\n**(C) @ruangprojects** ".format(plugin_name)
    await cb.edit_message_text(
        help_string,
        reply_markup=InlineKeyboardMarkup(
            [
