...
def load_modulesPyro(plugin_name, assistant=False):
    ...
    if plugin_name.endswith("__"):
        ...
        if plugin_name not in Config.MAIN_NO_LOAD:
            if assistant:
                plugin_path = "PunyaAlby.modules.assistant." + plugin_name
            else:
                plugin_path = "PunyaAlby.modules." + plugin_name
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        Path(f"PunyaAlby/modules/{shortname}.py")
        "PunyaAlby.modules.{}".format(shortname)
        logging.info("Successfully imported " + shortname)
    else:
        if plugin_path is None:
            Path(f"PunyaAlby/modules/{shortname}.py")
            f"PunyaAlby.modules.{shortname}"
        else:
...
def buka(folder):
    ...
    """
    path = f"PunyaAlby/modules/{folder}/*.py"
        ...
        with open(name) as f:
            ...
            try:
                if shortname.replace(".py", "") not in Config.NO_LOAD:
                    ...
                    while flag:
                        try:
                            load_module(
                                ...
                                plugin_path=f"PunyaAlby/modules/{folder}",
                            )
                            break
                    ...
                    os.remove(Path(f"PunyaAlby/modules/{folder}/{shortname}.py"))
