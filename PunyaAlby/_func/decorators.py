...
from ._helpers import edit_or_reply, is_admin_or_owner
...
def alby_cmd(
    ...
    def decorator(func):
        async def wrapper(client, message):
            ...
            if only_if_admin and not await is_admin_or_owner(message, (client.me).id):
                await edit_or_reply(
            ...
            if ignore_errors:
                ...
                try:
                    ...
                    text += "\n\n__You can Forward This to @ruangdiskusikami, If You Think This is Serious A Error!__"
                    try:
    ...
    def decorator(func):
        async def wrapper(client, message):
            try:
                ...
                text += f"\n**Trace Back : ** `{str(format_exc())}`"
                text += f"\n**ALBY-PYROBOT-Name :** `{func.__module__}`"
                text += f"\n**Function Name :** `{func.__name__}` \n"
                text += datetime_tz.strftime(
                    "**Date :** `%Y-%m-%d` \n**Time :** `%H:%M:%S`"
                )
                text += "\n\n__You can Forward This to @ruangdiskusikami, If You Think This is A Error!__"
                try:
                    await client.send_message(Config.LOG_GRP, text)
                except BaseException:
                    logging.error(text)
        ...
        if pyrobot3:
            pyrobot3.add_handler(MessageHandler(wrapper, filters=filter_s), group=0)
        if pyrobot4:
            pyrobot4.add_handler(MessageHandler(wrapper, filters=filter_s), group=0)
        return wrapper
