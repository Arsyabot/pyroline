...
class SqlDB(_BaseDatabase):
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS ALBY (albyCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('ALBY')) AS size"
        )
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM ALBY")
    ...
    def set(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE ALBY DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE ALBY ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO ALBY ({key}) values (%s)", (str(value),))
        return True

    def delete(self, key):
        try:
            self._cursor.execute(f"ALTER TABLE ALBY DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE ALBY")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS ALBY (albyCli varchar(70))"
        )
        return True
